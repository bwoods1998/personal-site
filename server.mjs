...
export function createServer(api, { built = false, origin = 'http://localhost:4173' } = {}) {
  ...
  return http.createServer({ requestTimeout: 15000, headersTimeout: 10000 }, async (req, res) => {
    ...
    try {
      ...
      const allowed = built ? /^(portfolio\/social\.png|index\.html|(?:admin|portfolio)\/index\.html|portfolio\/(?:snapshot|investigations|evaluations|replay|cashflow|scenario|project-status|universe|overnight-research|cash-map|agent-state)\.json|assets\/(?:(?:styles|portfolio|project)\.[a-f0-9]{12}\.css|(?:app|admin|chart|portfolio|investigations|evaluations|replay|cache|cashflow|scenario|universe|overnight|explorer|agent)\.[a-f0-9]{12}\.js|favicon\.[a-f0-9]{12}\.svg))$/ : /^(portfolio\/social\.png|index\.html|styles\.css|app\.js|chart\.js|favicon\.svg|admin\/(index\.html|admin\.js)|portfolio\/(index\.html|project\.css|portfolio\.(?:css|js)|(?:investigations|evaluations|replay|cache|cashflow|scenario|universe|overnight|explorer|agent)\.js|(?:snapshot|investigations|evaluations|replay|cashflow|scenario|project-status|universe|overnight-research|cash-map|agent-state)\.json))$/;
