...
export function createServer(api, { built = false, origin = 'http://localhost:4173' } = {}) {
  ...
  return http.createServer({ requestTimeout: 15000, headersTimeout: 10000 }, async (req, res) => {
    ...
    try {
      ...
      const allowed = built ? /^(index\.html|(?:admin|portfolio)\/index\.html|portfolio\/(?:snapshot|investigations|evaluations|replay|cashflow|scenario|project-status|universe)\.json|assets\/(?:(?:styles|portfolio)\.[a-f0-9]{12}\.css|(?:app|admin|chart|portfolio|investigations|evaluations|replay|cache|cashflow|scenario|universe)\.[a-f0-9]{12}\.js|favicon\.[a-f0-9]{12}\.svg))$/ : /^(index\.html|styles\.css|app\.js|chart\.js|favicon\.svg|admin\/(index\.html|admin\.js)|portfolio\/(index\.html|portfolio\.(?:css|js)|(?:investigations|evaluations|replay|cache|cashflow|scenario|universe)\.js|(?:snapshot|investigations|evaluations|replay|cashflow|scenario|project-status|universe)\.json))$/;
