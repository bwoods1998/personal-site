...
test('the page carries five sections in order, two numbers, the disclosure, no link and no external script', async () => {
  ...
  assert.doesNotMatch(floorHtml, /<a[\s>]/i, 'the page has no anchor element');
  assert.doesNotMatch(floorHtml.slice(floorHtml.indexOf('<body')), /href=|github|capital\/committee|capital\/desk/, 'nothing in the body points anywhere else');
...
test('the build publishes the floor with hashed, self-hosted assets', async () => {
  ...
  try {
    ...
    assert.doesNotMatch(floorHtml, /<a[\s>]/i, 'the built page has no anchor element');
...
test('the page says practice or shadow, never paper, and stills its motion on request', async () => {
  ...
  for (const gone of ['ladder', 'rows-arena', 'rows-leaders', 'partners-grid', 'navigation', 'footer-links', 'skip-link']) assert.ok(!css.includes(`.${gone}`), `${gone} styles are gone`);
});
