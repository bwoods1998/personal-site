...
import { PERFORMANCE_START_AT, mastheadNumbers, totalProfit, swarmRows, structureRows, feedLines, startCapital } from '../capital/capital.js';
...
test('published to a record and read back, the fixtures fill every section', { skip }, async () => {
  ...
  const [balance, profit, net] = mastheadNumbers(board, Date.parse(board.published_at));
  assert.notEqual(balance.value, '—');
  assert.equal(profit.value === '—', totalProfit(board) === null);
  assert.ok(net.value);
  ...
  await withBrowser('', path => capital.fetch(new Request('https://blakewoods.us' + path)), async () => {
    ...
    assert.ok(root.querySelector('#floor-swarm').find('tbody')[0].find('tr').length > 0);
    assert.equal(root.querySelector('#floor-structures').find('tbody')[0].find('tr').length, board.structures.length);
    assert.doesNotMatch(root.textContent, /kalshi|alpaca|coinbase/i);
    assert.match(words(root.querySelector('#floor-numbers')), /^Brokerage Account \$/);
  });
});
