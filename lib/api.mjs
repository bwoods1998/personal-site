...
export function createApi(exchange, { sessionSecret, adminKey }) {
  if (sessionSecret?.length < 32 || adminKey?.length < 8 || !sessionSecret || !adminKey) throw new Error('A session secret and admin password are required.');
  const sign = value => createHmac('sha256', sessionSecret).update(value).digest('base64url');
  const adminEpoch = sign(`admin-password:${adminKey}`).slice(0, 22);
  ...
  return async function api(request, networkAddress) {
    ...
    try {
      ...
      if (path === '/api/admin/login' && request.method === 'POST') {
        ...
        if (typeof input.key !== 'string' || !timingSafeEqual(hash(input.key), hash(adminKey))) throw new ApiError(401, 'Invalid admin password.');
        headers['Set-Cookie'] = cookie(request, 'bw_admin', `${await exchange.createAdminSession()}.${adminEpoch}`, 28800);
        return response({ ok: true });
      }
      if (path.startsWith('/api/admin/')) {
        const [token, epoch] = cookieValue(request, 'bw_admin').split('.');
        if (epoch !== adminEpoch) throw new ApiError(401, 'Sign in to review memos.');
