...
import {
  ...
  validEventBatch, validCheckpoint, validStream, validKind, parseStreamTags, socketMatches, deskId, tapeName,
...
export class Capital {
  ...
  // The owner's reset: the floor is being rebuilt, so the published tape, the balance history,
  // the checkpoint and the desks all go, and the page reads as a floor that has not started
  // (Sept 19, 2026). Guarded by the publish token and an explicit confirmation.
  resetAll() {
    const counts = {};
    this.transaction(() => {
      for (const table of ['events', 'floor_history', 'checkpoint', 'desks']) {
  ...
  async fetch(request) {
    ...
    if (path === '/api/capital/reset') {
      ...
      if (!authorized(request, this.env.CAPITAL_PUBLISH_TOKEN)) return reply({ error: 'Unauthorized.' }, 401);
      if (url.searchParams.get('confirm') !== 'erase-everything') return reply({ error: 'Add ?confirm=erase-everything to erase the published floor.' }, 400);
