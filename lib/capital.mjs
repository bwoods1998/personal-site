...
import {
  ...
  validEventBatch, validCheckpoint, validStream, validKind, parseStreamTags, socketMatches, agentId, tapeName,
...
export class Capital {
  ...
  readAgents(request) {
    ...
    return conditional(request, { schema_version: SCHEMA_VERSION, agents: rows.map(row => JSON.parse(row.body)) }, 5);
  }
